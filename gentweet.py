...
tweet.write('Cndtns at ''{}''\n''Tmp: ''{}''°F''\n''DwPt: ''{}''°F'.format(time[:-1],temp_str[:-7],dew[:-7]))
if chill_float < temp_float:
    tweet.write('\n''Wndchl: ''{}''°F'.format(chill_str[:-7]))
if heat_float > temp_float:
    tweet.write('\n''Ht indx: ''{}''°F'.format(heatindex_str[:-7]))
tweet.write('\n''Wnd spd: ''{}'' mph (''{}' '°, ''{}'')''\n''Mx gst past hr: ''{}'' mph''\n''Baro: ''{}'' in Hg (''{}'' in Hg/hr)'.form$
if float(hourlyrain[:-1])>0:
    tweet.write('\n''Hr rain: ''{}'' in'.format(hourlyrain[:-2]))
if float(dailyrain[:-1])>0:
    tweet.write('\n''Dly rain: ''{}'' in'.format(dailyrain[:-1]))
if float(solarrad[:-1])>0:
    tweet.write('\n''Slr rd: ''{}'' W/m^2'.format(solarrad[:-1]))
if float(uv[:-1])>0:
    tweet.write('\n''UV indx: ''{}'.format(uv[:-1]))
tweet.write('\n''Soil moist: ''{}'' centibar''\n''Soil temp: ''{}''°F''\n''Pool temp: ''{}''°F'.format(soilmoist[:-1],soiltemp3[:-1],po$
f.close()
tweet.close()
